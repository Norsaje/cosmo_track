...
Проверяют поведение, на которое опираются фронтенд и offline smoke-тесты.
Изменение состава `JobState` требует одновременного обновления потребителей.
...
#: валить тест, иначе зафиксированный API и код разъезжаются молча.
EXPECTED_PATHS = {
...
def test_reason_codes_stay_an_open_list(schema) -> None:
    """Список открыт: контракт C-09 версии 0.1 допускает новые коды,
