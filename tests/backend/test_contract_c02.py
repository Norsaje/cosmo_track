"""Contract-тест C-02: заглушка и загрузка модельного бандла.

Тест устроен в два слоя.

**Слой 1 — всегда.** Арифметика заглушки, классификация отказов бандла и правила
подмены проверяются без тяжёлых модельных зависимостей.
...
Префикс `COSMO_` здесь намеренно не используется: `conftest.py` вычищает
...
теста с таким префиксом исчезала бы до его тела:
    ...
    ML_CONTRACT_SRC=src uv run pytest -q tests/backend/test_contract_c02.py

Проверить полностью, включая реальный bundle, можно так:

    ML_CONTRACT_SRC=src ML_CONTRACT_BUNDLE=artifacts/ml/baseline_v1/bundle \\
