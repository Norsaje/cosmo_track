...
def test_declared_metrics_are_the_ones_we_may_quote(handoff) -> None:
    """Числа для README и демо берём из `handoff.json`, а не из чужих отчётов.
...
def test_temporal_transfer_is_worse_than_baseline(model) -> None:
    ...
    словами в README: режим C — перенос на будущий сезон, и там ансамбль проигрывает.
...
def test_quality_flags_reach_the_consumer(model, contracts) -> None:
    """README ML: «Backend должен сохранять diagnostics и показывать `quality_flags`,
    особенно для новых полигонов и слабого контекста». Полигон ниже модели незнаком,
