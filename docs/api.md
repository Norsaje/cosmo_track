...
`COSMO_REDIS_URL`, `COSMO_MODEL_PACKAGE_PATH`, `COSMO_CORS_ORIGINS`). Префикс не
...
- ### `GET /reference-polygons` — какие ряды доступны для анализа

Временный роут, пока не подключены живые провайдеры (BE-007/BE-009).

Нарисованный на карте контур сам по себе данных не несёт: геометрий в конкурсных
CSV нет, и связать полигон с рядом автоматически невозможно. Поэтому источник
выбирает человек, а интерфейс обязан показать, из чего выбирать — **до** запуска
анализа, а не после упавшей джобы.

~~~json
{"items": [{"anon_polygon_id": "AOI-0019", "observations": 938,
            "first_date": "2010-04-07", "last_date": "2024-10-30",
            "crop_type": "подсолнечник", "dataset": "train_dataset.csv"}],
 "total": 78}
~~~

Список отсортирован по числу наблюдений: полигоны из train идут раньше (медиана
767 наблюдений против 72 у test), и демо на них выглядит осмысленнее.
Выбранный ряд передаётся при создании поля в `properties.anon_polygon_id`.

`POST /analyses` **отказывает с 422**, если у поля нет такой привязки: анализ по
нему невозможен, и создавать заведомо падающую джобу незачем.
...
- **Модель** — монтируется из `MODEL_PACKAGE_HOST_PATH` (по умолчанию каталог
  поставки `./model`) в `/srv/model` только на чтение. Внутри: `ndvi/` — код
  инференса, `data/` — `train.csv` и `test_features.csv`, `runs/<COSMO_MODEL_RUN_NAME>/`
  — обученная смесь. Готовой (`ok`) модель объявляется только при наличии всех трёх:
  пустая точка монтирования даёт `degraded`, отсутствующая — `not_configured`.
  Обученная смесь лежит в pickle, поэтому загружается только при
  `COSMO_MODEL_BUNDLE_TRUSTED=true` — pickle исполняет код, и доверие включается
  после сверки хешей `PACKAGE_MANIFEST.json`, а не «чтобы заработало».
  Ряды для offline-источника берутся оттуда же (`COSMO_MODEL_DATA_DIR`): сервис
  обязан отдавать ровно те полигоны, на которых обучалась модель.
