"""Data quality report for train/test CSVs (Developer 4, cosmo_track ED).
...
Контракт см. docs/ed_part.md §"Задача 1".
"""
...
# Целевые эталоны из ed_part.md §"Часть 1, раздел 4".
...
def _is_finite(value: Any) -> bool:
    ...
    Используем именно math.isfinite, как требует ed_part.md.
    """
    if value is None:
        return False
...
def compute_quality(
    ...
    *,
    ...
    иначе нарушается детерминизм (см. ed_part.md).
    """
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    """Пишет Markdown по 7 разделам из ed_part.md §"Markdown output"."""
    train = report["train"]
    test = report["test"]
    lines: list[str] = []
...
def parse_args(argv: list[str] | None = None) -> Args:
    ...
    # Абсолютные пути запрещены (см. ed_part.md §"ОБЩИЕ ПРАВИЛА").
...
def main(argv: list[str] | None = None) -> int:
    ...
    for col in ("anon_polygon_id", "date"):
        if col not in train_df.columns:
            _warn_and_stderr(
                ...
                f"  как исправить: приведите train к ожидаемой схеме (см. docs/ed_part.md)"
            )
            return 2
        if col not in test_df.columns:
            _warn_and_stderr(
                f"[error] test CSV missing required column: {col!r}\n"
                f"  как исправить: приведите test к ожидаемой схеме (см. docs/ed_part.md)"
            )
            return 2
    ...
    if has_mismatch and not args.allow_mismatch:
        ...
        _warn_and_stderr(
            ...
            + "\n  как исправить: см. docs/ed_part.md §\"Часть 1, раздел 4\" "
            "— НЕ правьте CSV, сообщите reviewer.\n"
