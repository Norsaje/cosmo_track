"""Safe fixture builder for tests and demo (Developer 4, cosmo_track ED).
...
Контракт см. docs/ed_part.md §"Задача 3".
"""
...
# которые НЕ смешиваются с копиями реальных данных (см. ed_part.md
# §"Задача 3: Состав").
TOY_PREFIX = "TOY-"
...
def write_csv(df: pd.DataFrame, path: Path) -> None:
    """Пишет CSV. index=False (см. ed_part.md §"ПРАВИЛА СКРИПТОВ")."""
    path.parent.mkdir(parents=True, exist_ok=True)
