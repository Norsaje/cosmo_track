"""Strict submission validator (Developer 4, cosmo_track ED).

Независимая страховка для submission.csv. НЕ редактирует submission,
НЕ авто-фиксит неверные prediction. Только проверяет по 10 правилам из
docs/ed_part.md §"Задача 2" и формирует понятный отчёт.

Контракт см. docs/ed_part.md §"Задача 2".
"""
...
EXPECTED_ROW_COUNT = 3_112  # число gaps в текущем test (см. ed_part.md §"4")
...
def _is_finite(value: Any) -> bool:
    """Является ли значение конечным числом (см. ed_part.md §"ОБЩИЕ ПРАВИЛА")."""
    if value is None:
        return False
...
def _check_predictions_range(
    ...
    По ed_part.md НЕ отклоняет submission, только предупреждает.
    """
...
def _read_submission(path: Path) -> tuple[pd.DataFrame, str | None]:
    ...
    # 2) Проверяем delimiter. По ed_part.md — строго запятая.
