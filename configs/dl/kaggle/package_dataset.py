...
В штатной раскладке ML и DL находятся в одном дереве, и в архив входит `src/`.
Раскладка записывается в `BUNDLE_MANIFEST.json` полем `layout`.
...
он содержит только `train_dataset.csv`, уже опубликованный обеими ветками.
    ...
    python configs/dl/kaggle/package_dataset.py \
        --ml-root . --inputs artifacts/dl/c03_derived \
