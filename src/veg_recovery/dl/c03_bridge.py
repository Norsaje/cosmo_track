...
`MaskSpec.from_test`, `load_folds`, `split_fold`) из указанного worktree и
экспортирует ровно те ключи, которые вернул producer. Файлы ML не изменяются и
не копируются в DL-реализацию.
...
приходят только от ML. `review_status` остаётся
`derived_from_producer_artifacts`: подтверждение ML всё ещё требуется.
"""
