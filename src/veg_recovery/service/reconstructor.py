...
Модель не дублируется: batch и веб-путь вызывают один и тот же
...
Импорты `veg_recovery.contracts` и `inference` ленивые, чтобы базовый пакет
работал в лёгком окружении без модельных зависимостей. Контракты
`ReconstructionRequest` и `ReconstructionResult` при этом не дублируются.
"""
...
#: Классификация отказов `load_bundle` по тексту исключения. Загрузчик поднимает
...
class ModelContractMissing(RuntimeError):
    """Пакет `veg_recovery.contracts` недоступен в текущем окружении."""
...
class ModelStub:
    ...
    Содержимое — простой baseline «mean two neighbors». Заглушка используется
    только в разрешённом контуре разработки и явно маркируется в результате.
    ...
    def predict(self, request: Any) -> Any:
        ...
        if not np.isfinite(predicted).all():
            # Никакой fallback не имеет права вернуть NaN.
...
def load_ml_bundle(bundle_path: str | Path, *, trusted: bool = False) -> LoadedReconstructor:
    """Загрузить совместимый бандл C-03 baseline_v1 или C-04 p0-catboost-gpu-v1.

    Веб-путь его больше не зовёт: с BE-011R модель приходит из каталога `model/`.
    Функция остаётся точкой проверки обратной совместимости. В `build_reconstructor`
...
def build_reconstructor(
    ...
    *,
    ...
    для проверки совместимости, но веб-путь его больше не зовёт — двух моделей
