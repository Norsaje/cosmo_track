...
#: Значение из `RESULTS.md` поставки; расхождение с файлом помечается флагом.
...
class NdviRunReconstructor:
    ...
    def predict(self, request: Any) -> Any:
        ...
        if not np.isfinite(predicted).all():
            ...
            # ни до БД, ни до графика (`docs/01_ml_developer.md:315`).
    ...
    def _source_probabilities(self, features: pd.DataFrame) -> _Sources:
        ...
        измеренная точность по трём классам — 95.6–98.2 % (`RESULTS.md`).
