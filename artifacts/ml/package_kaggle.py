"""Create deterministic Kaggle archives for GPU and CPU training notebooks."""
...
DESTINATIONS=(
    ROOT/'artifacts/ml/kaggle_ndvi_gpu_v1.zip',
    ROOT/'artifacts/ml/kaggle_ndvi_cpu_trees_v1.zip',
)
...
def main():
    ...
    for relative in ['data/train_dataset.csv','data/test_data.csv','docs/case_doc.pdf','docs/criteria.pdf',
                     'artifacts/ml/evaluate_baselines.py','artifacts/ml/package_kaggle.py',
    ...
    for dest in DESTINATIONS:
        ...
        results.append({'archive':str(dest),'bytes':dest.stat().st_size,
                        'files':len(selected),'sha256':digest})
