...
def _detect_anomalies(session, analysis: Analysis, series, outcome: AnalysisOutcome) -> list[str]:
    """Аномалии baseline-детектором при недоступности реализации C-09.
    ...
    warnings = [
        "ANOMALY_SOURCE_IS_BASELINE: события получены baseline-детектором, "
        "а не реализацией C-09; категории severity (normal/biomass_suppression/"
