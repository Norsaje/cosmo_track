...
class SeriesPoint(BaseModel):
    ...
    #: Производится не нами — владение уточняется у teamlead (ML-014 против DL).
...
class AnomalyOut(BaseModel):
    """Событие аномалии в терминах контракта C-09 (владелец — Разработчик 2, DL).

    Мы это событие только храним и отдаём, логику детектора к себе не переносим.
    """
    ...
    #: ОТКРЫТЫЙ список строк. В ТЗ DL перечень кодов нигде не объявлен исчерпывающим,
    #: C-09 сейчас 0.1/NOT_STARTED. Поэтому здесь запрещены Literal/Enum, а в БД —
...
class AnomaliesResponse(BaseModel):
    ...
    `DetectionResult` у DL несёт warnings отдельно от событий, и его докстрока прямо
    гласит: «quality warnings отдельно от отсутствия аномалий». Без них пустой `items`
    означал бы одновременно «поле в норме» и «данных не было» — а ТЗ DL запрещает
    трактовать отсутствие события при недостатке данных как подтверждённую норму.
    """
